    ...
    if (!lock_clust_rec_cons_read_sees(clust_rec, index, offsets,
      ...
      err =
          row_sel_build_prev_vers(node->read_view, index, clust_rec, &offsets,
                                  &heap, &plan->old_vers_heap, &old_vers, mtr,/*DBS*/trx);
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
  ...
  //fprintf(stderr, "checkpoint4 orw_sel_build_prev_vers_for_mysql %lu\n",trx->id);
  err = row_vers_build_for_consistent_read(
      rec, mtr, clust_index, offsets, read_view, offset_heap,
      prebuilt->old_vers_heap, old_vers, vrow, lob_undo,/*DBS*/ trx);
...
dberr_t Row_sel_get_clust_rec_for_mysql::operator()(
  ...
  *offsets = rec_get_offsets(clust_rec, clust_index, *offsets, ULINT_UNDEFINED,
  ...
  if (prebuilt->select_lock_type != LOCK_NONE) {
	...
	fprintf(stderr, "checkpoint5 operator %lu\n",trx->id);
        err = row_sel_build_prev_vers_for_mysql(
            trx->read_view, clust_index, prebuilt, clust_rec, offsets,
            offset_heap, &old_vers, vrow, mtr, lob_undo,trx/*DBS*/);
...
dberr_t row_search_mvcc(byte *buf, page_cur_mode_t mode,
  ...
  it. */
  //fprintf(stderr, "checkpoint8 row_search_mvcc trx_id : %lu, read_write : %d, thread_id : %u \n",trx->id,trx->read_write,thd_thread_id(trx->mysql_thd));
  ...
  locks_ok:
    ...
    if (trx->isolation_level == TRX_ISO_READ_UNCOMMITTED) {
      ...
      if (srv_force_recovery < 5 &&
        ...
        rec_t *old_vers;
        /* The following call returns 'offsets' associated with 'old_vers' */
	//fprintf(stderr, "checkpoint6 row_search_mvcc trx_id : %lu read_only : %d read_write : %d, thread_id : %u\n",trx->id,trx->read_only, trx->read_write, thd_thread_id(trx->mysql_thd));
        err = row_sel_build_prev_vers_for_mysql(
            trx->read_view, clust_index, prebuilt, rec, &offsets, &heap,
            &old_vers, need_vrow ? &vrow : nullptr, &mtr,
            prebuilt->get_lob_undo(),trx/*DBS*/);
