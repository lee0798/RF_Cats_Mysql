...
dberr_t row_vers_build_for_consistent_read(
    ...
    rec_t **old_vers, const dtuple_t **vrow, lob::undo_vers_t *lob_undo,/*DBS*/trx_t* trx) {
  DBUG_TRACE;
  const rec_t *version;
  rec_t *prev_version;
  trx_id_t trx_id;
  ...
  for (;;) {
    ...
    //DBS-start
    temp_page = page_align(version);
   
    if (trx != nullptr) {
	    	const LEX_CSTRING& query_string = trx->mysql_thd->query();
		fprintf(stderr, "User Query : %s\n",query_string.str);
	        fprintf(stderr, "Current Space Id: %u, Page No: %u, read_trx_id: %lu read_only:%d, read_write : %d, thread_id : %u \n",
				           page_get_space_id(temp_page), page_get_page_no(temp_page), trx->id,trx->read_only, trx->read_write, thd_thread_id(trx->mysql_thd));

    } else {
	        fprintf(stderr, "Error: trx is nullptr.\n");
    }
    //DBS-end
